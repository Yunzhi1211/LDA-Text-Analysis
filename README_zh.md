...
### Q1: 运行出错 "UnicodeDecodeError"

**解决**: 检查DOCX文件编码，确保是UTF-8格式

### Q2: jieba分词效果不理想

**解决**: 添加自定义词汇。在4_scripts/1_preprocessing.py中修改：

```python
import jieba
jieba.load_userdict('my_dict.txt')  # 自定义词汇表
```

### Q3: 主题太多/太少

**解决**: 调整 `0_config/config.yaml` 中的 `num_topics`

### Q4: 运行速度慢

**解决**: 减少 `passes` 和 `iterations` 参数

### Q5: 内存不足

**解决**: 增加 `min_df` 或减少 `max_df` 来减少词汇量
...
✅ `src/` - 所有代码  
✅ `0_config/` - 配置模板  
✅ `2_output/lda_results.json` - 脱敏结果  
✅ `2_output/visualizations/` - 图表  
✅ README.md、requirements.txt  

❌ `1_input/` - 原始数据（隐私）  
❌ `lda_analysis.log` - 运行日志  
...
本项目作为**乡村振兴调研计划**的一部分而开发。主要贡献：

| 角色 | 贡献者 |
|------|-----|
| **LDA主题分析**（主要贡献人） | **韩君怡** 🌟 |
| 数据分析与研究 | 韩君怡、郝佳锐、岳文杰、吴依诺 |
| 调查采访 | 江昀芷、张文莹、刘秦艳、章萍萍、黎夏怡、李函洁、夏颖 |
| 宣传支持 | 蒋梦钰、曹雨婷、谭婷婷、李跃星、邓韵然 |
